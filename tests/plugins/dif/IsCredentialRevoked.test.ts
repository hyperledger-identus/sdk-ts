...
describe("Plugins - DIF", () => {
  describe("IsCredentialRevoked", () => {
    ...
    it("Should throw an error if we try to use a SDJWT credential ", async () => {
      ...
      expect(sut).rejects.toThrow(`Only JWT Credential are supported`);
    });
    //*
    ...
    it("Should throw an error if we try to use a credential which an unsupported type ", async () => {
      ...
      expect(sut).rejects.toThrow(`CredentialStatus revocation type not supported`);
    });
    ...
    it("Should throw an error if the credential status proof contains an invalid verificationMethod", async () => {
      ...
      await expect(sut).rejects.toThrow(`CredentialStatus proof invalid verificationMethod`);
    ...
    it("should throw an error if no jwk is provided", async () => {
      ...
      await expect(sut).rejects.toThrow("No public jwk provided");
    });
    ...
    it("should throw an eror if a wrong verificationMethod type is used", async () => {
      ...
      await expect(sut).rejects.toThrow("Err Only EcdsaSecp256k1VerificationKey2019 is supported");
    });
    ...
    it("Should throw an error if a wrong key type is used", async () => {
      ...
      await expect(sut).rejects.toThrow("Err Invalid JWK kty: undefined, should be EC");
    ...
    it("Should throw an error if a wrong key curve is used", async () => {
      ...
      await expect(sut).rejects.toThrow("Err Invalid JWK crv: undefined, should be secp256k1");
    });
    ...
    it("Should throw an eror if an invalid verificationKey is used", async () => {
      ...
      await expect(sut).rejects.toThrow("CredentialStatus proof invalid verifying key");
    });
    ...
    it("Should throw an error if the status jwt is invalid", async () => {
      ...
      await expect(sut).rejects.toThrow("Credential status jwt is invalid");
    });
    ...
    it("should throw an error if wrong signature is used", async () => {
      ...
      await expect(sut).rejects.toThrow("CredentialStatus invalid signature");
    });
    ...
    it("Should throw an error if the status proof type is invalid", async () => {
      ...
      await expect(sut).rejects.toThrow("CredentialStatus proof type not supported");
    });
    //*/
  });
});
