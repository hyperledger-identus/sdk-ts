 ...
 */
export class FetchAuthServerMeta extends Task<Domain.ApiResponse<AuthServerMetadata>, FetchAuthServerMetaArgs> {
  async run(ctx: Context) {
    ...
    const response = await ctx.Api.request("GET", url.href, undefined, new Map(Object.entries(headers)));
  ...
  private validate(response: Domain.ApiResponse): asserts response is Domain.ApiResponse<AuthServerMetadata> {
    validate(response.body, AuthServerMetadata);
  }
}
