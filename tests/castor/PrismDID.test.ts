...
describe("PrismDID", () => {
  ...
  describe("PrismDidPublicKey", () => {
    it("Should create getProtosUsageCorrectly", () => {
      expect(getProtosUsage("any" as any)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.UNKNOWN_KEY);
      expect(getProtosUsage(Usage.MASTER_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.MASTER_KEY);
      expect(getProtosUsage(Usage.ISSUING_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.ISSUING_KEY);
      expect(getProtosUsage(Usage.KEY_AGREEMENT_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.KEY_AGREEMENT_KEY);
      expect(getProtosUsage(Usage.AUTHENTICATION_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.AUTHENTICATION_KEY);
      expect(getProtosUsage(Usage.REVOCATION_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.REVOCATION_KEY);
      expect(getProtosUsage(Usage.CAPABILITY_INVOCATION_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.CAPABILITY_INVOCATION_KEY);
      expect(getProtosUsage(Usage.CAPABILITY_DELEGATION_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.CAPABILITY_DELEGATION_KEY);
      expect(getProtosUsage(Usage.UNKNOWN_KEY)).toEqual(Protos.io.iohk.atala.prism.protos.KeyUsage.UNKNOWN_KEY);
    });
    ...
    it("Should create from and to valid key protos", () => {
      ...
      keys.forEach((key) => {
        ...
        expect(masterPk.keyData.raw).toStrictEqual(recoveredPk.keyData.raw);
        expect(masterPk.usage).toEqual(recoveredPk.usage);
        expect(masterPk.id).toEqual(recoveredPk.id);
      });
      ...
      expect(() => PrismDIDPublicKey.fromProto(apollo, masterPkProto)).toThrow(`16: Invalid key curve: ${unsupportedCurve}. Valid options are: X25519, Ed25519, secp256k1`);
    });
  });
  ...
  describe("Integration Tests", () => {
    describe("DIDDocument", () => {
      test("real example resolves correctly", async () => {
        ...
        expect(sut.coreProperties).to.be.an("array").toHaveLength(4);

        const cp0 = sut.coreProperties.at(0) as Authentication;
        expect(cp0).toBeInstanceOf(Authentication);
        expect(cp0.urls).to.include(`${didStr}#master-0`);
        const cp0vm0 = cp0.verificationMethods.at(0);
        expect(cp0vm0).toBeInstanceOf(VerificationMethod);
        expect(cp0vm0?.controller).toEqual(didStr);
        expect(cp0vm0?.id).toEqual(`${didStr}#master-0`);
        expect(cp0vm0?.publicKeyJwk).toBeUndefined();
        expect(cp0vm0?.publicKeyMultibase).toEqual("zSXxpYB6edvxvWxRTo3kMUoTTQVHpbNnXo2Z1AjLA78iqLdK2kVo5xw9rGg8uoEgmhxYahNur3RvV7HnaktWBqkXt");
        expect(cp0vm0?.type).toEqual("EcdsaSecp256k1VerificationKey2019");

        const cp1 = sut.coreProperties.at(1) as Authentication;
        expect(cp1).toBeInstanceOf(Authentication);
        expect(cp1.urls).to.include(`${didStr}#authentication-0`);
        const cp1vm0 = cp1.verificationMethods.at(0);
        expect(cp1vm0).toBeInstanceOf(VerificationMethod);
        ...
        expect(cp2).toBeInstanceOf(Services);
        expect(cp2.values).toHaveLength(0);

        // TODO why do we have duplicates of coreProperties 0 & 1 here?
        const cp3 = sut.coreProperties.at(3) as VerificationMethods;
        expect(cp3).toBeInstanceOf(VerificationMethods);
        expect(cp3.values).toHaveLength(2);

        const cp3v0 = cp3.values.at(0);
        expect(cp3v0).toBeInstanceOf(VerificationMethod);
        expect(cp3v0?.controller).toEqual(didStr);
        expect(cp3v0?.id).toEqual(`${didStr}#master-0`);
        expect(cp3v0?.publicKeyJwk).toBeUndefined();
        expect(cp3v0?.publicKeyMultibase).toEqual("zSXxpYB6edvxvWxRTo3kMUoTTQVHpbNnXo2Z1AjLA78iqLdK2kVo5xw9rGg8uoEgmhxYahNur3RvV7HnaktWBqkXt");
        expect(cp3v0?.type).toEqual("EcdsaSecp256k1VerificationKey2019");

        const cp3v1 = cp3.values.at(1);
        expect(cp3v1).toBeInstanceOf(VerificationMethod);
        expect(cp3v1?.controller).toEqual(didStr);
        expect(cp3v1?.id).toEqual(`${didStr}#authentication-0`);
        expect(cp3v1?.publicKeyJwk).toBeUndefined();
        expect(cp3v1?.publicKeyMultibase).toEqual("zSXxpYB6edvxvWxRTo3kMUoTTQVHpbNnXo2Z1AjLA78iqLdK2kVo5xw9rGg8uoEgmhxYahNur3RvV7HnaktWBqkXt");
        expect(cp3v1?.type).toEqual("EcdsaSecp256k1VerificationKey2019");
      });
      ...
      const testVerificationMethod = (sut: any, didStr: string, keyId: string, keyMultibase: string, curve: Curve) => {
        expect(sut).toBeInstanceOf(VerificationMethod);
        ...
        expect(sut?.type).toEqual(getCurveVerificationMethodType(curve));
      };
      ...
      test("master key", async () => {
        ...
        expect(masterProp).toBeInstanceOf(Authentication);
        expect(masterProp.urls[0]).toEqual(`${secpDid}#${masterKeyId}`);
        ...
        expect(authProp).toBeInstanceOf(Authentication);
        ...
        expect(services.values).toHaveLength(0);
        ...
        expect(verificationMethods).toBeInstanceOf(VerificationMethods);
      ...
      test("issuing keys", async () => {
        ...
        expect(sut.coreProperties).to.be.an("array").toHaveLength(5);
