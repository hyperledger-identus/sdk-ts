...
export class ParseInvitation extends Task<InvitationType, Args> {
  ...
  private decode() {
    if (this.args.value instanceof URL) {
      return expect(this.tryDecodeUrl(this.args.value), AgentError.InvitationIsInvalidError);
    }
  ...
  private tryDecodeUrl(value: string | URL) {
    try {
      ...
      const oob = expect(url.searchParams.get("_oob"), AgentError.InvalidURLError);
