...
function wasmPackagesPlugin(): Plugin {
  const aliases: Record<string, string> = {
    '@hyperledger/identus-anoncreds-wasm': generatedDir,
  }

  return {
    name: 'wasm-packages',
    setup(build) {
      // Resolve subpath imports like @hyperledger/identus-anoncreds/foo_bg.wasm
      build.onResolve({ filter: /^@hyperledger\/identus-anoncreds-wasm\// }, (args) => {
      ...
      // Resolve bare package imports like @hyperledger/identus-anoncreds
